...
class wp_sherif_conversion_frontend_display{
	...
	static function init(){
		...
		add_shortcode('Campaign_Code', array(get_class(), 'replace_shortcode_with_campaign'));
	...
	/*
	 * parse the shortcode to get a campaign
	 * */
	static function replace_shortcode_with_campaign($attr){
		$cam = '';
		$camp_id = (int) $attr['id'];
		if($camp_id){
			$cam = self::get_campaign_content($camp_id);
		}
	...
	static function get_campaign_content($post_id){
		...
		$primary_content = $wpdb->get_var("SELECT post_content FROM $wpdb->posts WHERE ID = $post_id ");
				
		return (isset($_COOKIE[$name])) ? get_post_meta($post_id, 'secondary_campaign_content', true) : $primary_content;
	}
